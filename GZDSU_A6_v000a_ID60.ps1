...
<#
function __OLD__Invoke-ExtremelyElegantFix {
...
function Invoke-ExtremelyElegantFix_version2 {
    Write-Host "`n////////////////////////////////////////////////" @script:warningColors
    Write-Host "[ !! CAUTION !! ] - Hacky bullshit commencing..." @script:warningColors
    Write-Host "////////////////////////////////////////////////" @script:warningColors
    ...
    # Try to find and remove the actor definition line
    if ($zscriptContent -match "$script:__fixed__UserDefinedActorName") {
        Write-Host "`n[ - DEBUG - ] - Located target actor definition in zscript.txt" @script:debugColors
        $zscriptContent = $zscriptContent -replace "$script:__fixed__UserDefinedActorName", ''
        Set-Content -Path $zscriptPath -Value $zscriptContent
        Write-Host "[ - DEBUG - ] - Deleted target actor definition from zscript.txt" @script:debugColors
    } else {
        Write-Host "`n[ - DEBUG - ] - Target actor definition not found in zscript.txt" @script:debugColors
    }

    # ---- Create 'actors' folder and new actor file ----
    $actorsFolderPath = Join-Path -Path "$script:project_root_directory\zscript" -ChildPath "actors"
    if (-not (Test-Path $actorsFolderPath)) {
        New-Item -Path $actorsFolderPath -ItemType Directory | Out-Null
        Write-Host "[ - DEBUG - ] - Created 'actors' directory." @script:debugColors
    }

    # Create and populate actor file
    $actorFilePath = Join-Path -Path $actorsFolderPath -ChildPath "$script:__fixed__UserDefinedActorName.zs"
    $currentDate = Get-Date
    Set-Content -Path $actorFilePath -Value "// Auto-generated by $($script:full_title) at $currentDate"
    Add-Content -Path $actorFilePath -Value "$script:__recovered__ActorDefinition"
    Write-Host "[ - DEBUG - ] - Written actor definition to $actorFilePath" @script:debugColors

    # ---- Append include statement to zscript.txt ----
    Add-Content -Path $zscriptPath -Value "`n// ZZ_automation_recognition_token___KEEP_THIS___--___DO_NOT_DELETE___"
    Add-Content -Path $zscriptPath -Value "// Auto-generated by $($script:full_title) at $currentDate"
    Add-Content -Path $zscriptPath -Value "#include `"zscript/actors/$script:__fixed__UserDefinedActorName.zs`"`n"
    Write-Host "[ - DEBUG - ] - Appended include statement to zscript.txt" @script:debugColors

    Write-Host "`n[ - OK - ] - Elegant fix operation completed." @script:successColors
}

function Remove-ZscriptContentAboveKillToken {
    Write-Host "`nProcessing zscript.txt for content removal..." @script:warningColors
    ...
    # ---- identify the index of the token line ----
    $tokenIndex = $existingContent.IndexOf("// ZZ_automation_recognition_token___NUKE_THIS_UP___")

    if ($tokenIndex -eq -1) {
        Write-Host "[ - WARNING - ] - Token line not found in zscript.txt" @script:warningColors
        return
    }

    # ---- get the content below the token line ----
    $updatedContent = $existingContent[($tokenIndex + 1)..($existingContent.Length - 1)]

    # ---- write the updated content back to zscript.txt ----
    Set-Content -Path $zscriptPath -Value $updatedContent
    Write-Host "[ - OK - ] - Removed specified content from zscript.txt" @script:successColors

    # ---- mini-function to delete specified files and folders ----
    function Debug-CleanUpZscriptArtifacts {
        $zscriptAutoFile = Join-Path -Path "$script:project_root_directory\zscript" -ChildPath "zscript_auto.zs"
        $zscriptActorsFolder = Join-Path -Path "$script:project_root_directory\zscript" -ChildPath "zscript_actors_auto"

        if (Test-Path $zscriptAutoFile) {
            Remove-Item -Path $zscriptAutoFile -Force
            Write-Host "[ - OK - ] - Deleted zscript_auto.zs" @script:successColors
        } else {
            Write-Host "[ - INFO - ] - zscript_auto.zs not found." @script:infoColors
        }

        if (Test-Path $zscriptActorsFolder) {
            Remove-Item -Path $zscriptActorsFolder -Recurse -Force
            Write-Host "[ - OK - ] - Deleted zscript_actors_auto folder" @script:successColors
        } else {
            Write-Host "[ - INFO - ] - zscript_actors_auto folder not found." @script:infoColors
        }
    }

    # ---- invoke the mini-function ----
    Debug-CleanUpZscriptArtifacts

    # ---- replace the old token with the new one ----
    (Get-Content $zscriptPath) -replace '// ZZ_automation_recognition_token___NUKE_THIS_UP___', '// ZZ_automation_recognition_token___ZSCRIPT_ROOT___' | Set-Content $zscriptPath
    Write-Host "[ - OK - ] - Token line replaced in zscript.txt" @script:successColors
}
...
Invoke-ExtremelyElegantFix_version2
Remove-ZscriptContentAboveKillToken
